...
extension View {
    ...
    ///   - textStyle: A text style builder that returns the new text style to use for the given key
    /// path.
    ...
    /// Replaces a specific block style on the current ``Theme`` with a block style initialized with
    /// the given body closure.
    /// - Parameters:
    ...
    /// - Parameters:
    ///   - keyPath: The ``Theme`` key path to the block style to replace.
    ///   - body: A view builder that receives the block configuration and returns the customized
    /// block.
