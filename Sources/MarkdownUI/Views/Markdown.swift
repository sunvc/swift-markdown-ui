...
///
/// A `Markdown` view displays rich structured text using the Markdown syntax. It is compatible with
/// the
/// [GitHub Flavored Markdown Spec](https://github.github.com/gfm/) and can display
/// images, headings, lists (including task lists), blockquotes, code blocks, tables, and thematic
/// breaks,
...
/// initializer, which takes a Markdown content builder in which you can compose the view content,
/// either by providing Markdown strings
...
///
/// You can also create a ``MarkdownContent`` value in your model layer and later create a
/// `Markdown` view by passing
/// the content value to the ``Markdown/init(_:baseURL:imageBaseURL:)-42bru`` initializer. The
/// ``MarkdownContent``
/// value pre-parses the Markdown string preventing the view from doing this step.
///
/// ```swift
/// // Somewhere in the model layer
/// let content = MarkdownContent("You can try **CommonMark**
/// [here](https://spec.commonmark.org/dingus/).")
///
...
/// ### Styling Markdown
///
/// Markdown views use a basic default theme to display the contents. For more information, read
/// about the ``Theme/basic`` theme.
///
/// ```swift
/// Markdown {
///   """
...
/// `markdownTheme(_:)` modifier.
/// For example, you can apply one of the built-in themes, like ``Theme/gitHub``, to either a
/// Markdown view or a view hierarchy that
...
/// ![](GitHubBlockquote)
///
/// To override a specific text style from the current theme, use the
/// `markdownTextStyle(_:textStyle:)`
...
/// ![](CustomInlineCode)
///
/// You can also use the `markdownBlockStyle(_:body:)` modifier to override a specific block style.
/// For example, you can
...
/// ![](CustomBlockquote)
///
/// Another way to customize the appearance of Markdown content is to create your own theme. See the
/// documentation of the
/// ``Theme`` type for more information on this subject.
///
/// ### Customizing link behavior
///
/// When a user taps or clicks on a Markdown link, the default behavior is to open Safari. However,
...
public struct Markdown: View {
    ...
    @State private var blockStyles: [BlockNode.BlockType: BlockStyleAttributes] = [:]
    ...
    ///   - baseURL: The base URL to use when resolving Markdown URLs. If this value is `nil`, the
    /// initializer will consider all
    ///              URLs absolute. The default is `nil`.
    ///   - imageBaseURL: The base URL to use when resolving Markdown image URLs. If this value is
    /// `nil`, the initializer will
    ...
    public var body: some View {
        TextStyleAttributesReader { attributes in
            BlockSequence(self.blocks.coalesced())
...
extension Markdown {
    ...
    ///
    /// Using this initializer, you can compose the Markdown view content either by providing
    /// Markdown strings or with an expressive
    ...
    /// `nil`, the initializer will
    ///                   determine image URLs using the `baseURL` parameter. The default is `nil`.
    ///   - content: A Markdown content builder that returns the blocks that form the Markdown
    /// content.
    public init(
        baseURL: URL? = nil,
        imageBaseURL: URL? = nil,
